...
class TestKauppa(unittest.TestCase):
    ...
    def test_maksettaessa_tuote_joka_on_loppu_pankin_metodia_tilisiirto_kutsutaan_oikeilla_parametreilla(self):
        pankki_mock = Mock()
        viitegeneraattori_mock = Mock()
    ...
    def test_jokaiselle_maksutapahtumalle_pyydetaan_uusi_viitenumero(self):
        ...
        # palautetaan eri arvot kutsuilla
        ...
        # tehdään ostokset ja maksut kolme kertaa
        for _ in range(3):
            kauppa.aloita_asiointi()
            kauppa.lisaa_koriin(1)
            kauppa.tilimaksu("pekka", "12345")

        # varmistetaan, että viitenumeroita on pyydetty kolme kertaa
        self.assertEqual(viitegeneraattori_mock.uusi.call_count, 3)

    def test_poistettaessa_tuote_tilisiirtoa_kutsutaan_oikein(self):
        pankki_mock = Mock()
        viitegeneraattori_mock = Mock()
